...
**** Perf & Flamegraph
     ...
     As expected, most of the time is spent handling IO, specifically related to the =EventLog=. There are several options to reduce the time taken by =EventLog=, the easiest being to remove =std::endl= from =AppendEvent(...)=.
