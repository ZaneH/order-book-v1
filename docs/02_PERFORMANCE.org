...
**** Perf & Flamegraph
     All flamegraphs have been exported as SVGs. If you're viewing these on GitHub, I recommend you open the image in a new tab.
...
*** Optimizing
     As expected, most of the time is spent handling IO, specifically related to the =EventLog=. I thought the simplest speed-up would come from removing =std::endl= from =AppendEvent(...)=. After making the change, it had a negligible impact.
