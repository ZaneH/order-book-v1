...
- [ ] CMake builds:
  ...
  - tests target
- [ ] Pick compile flags and stick to them:
  - warnings on, treat warnings as errors in CI
  - debug build uses sanitizers (ASAN/UBSAN)
- [ ] Add clang-format config (even a minimal one) and actually run it.
- [ ] Add a README with the one command to build and run.
...
** tasks
- [ ] Write a short glossary in a NOTES.org (or in this file):
  - side, tick, level, FIFO, best bid/ask, spread, cross
- [ ] Do at least 3 hand-sim scenarios in your own words.
